...
\begin{document}
	...
	\begin{tikzpicture}[node distance=0.5cm,auto]
		...
		\node[below = 0.5 cm of gravid,mynode](egg){Egg};
		\draw[myarrow1] (gravid) -- (egg);
		\node[below = 0.5 cm of egg,mynode](hatched){Eclosed larva};
		\draw[myarrow1] (egg) -- (hatched);
		\node[below = 0.5 cm of hatched,mynode](hardening){Hardening larva};
		\draw[myarrow2] (hatched) -- (hardening);
		\node[below = 0.5 cm of hardening,mynode](active){Active larva};
		\draw[myarrow2] (hardening) -- (active);
		\node[below right = 0.25 cm of active,mynode](winter){Overwintering larva};
		\draw[myarrow1] (active) edge[bend left = 40] (winter);
		\draw[myarrow1] (winter) edge[bend left = 40] (active);
		\node[right = 1.75 cm of winter,mynode](dummy1){};
		\draw[myarrow2] (winter) -- (dummy1) node[midway, text width = 6em, xshift = 0.25cm]{Overwinter mortality};
		\node[below left = 0.25 cm of active,mynode](quest){Questing larva};
		...
		\draw[myarrow2, in = 90, out = 0] (hatched) to node[midway]{Diapause} (winter);
